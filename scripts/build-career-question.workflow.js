...
Always EMPOWERING and AGENTIC — work is something the querent acts on, not fate: tell the truth AND what they can do about it (e.g. "…and the move is yours to make" / "…และจังหวะนี้คุณเป็นคนกำหนดเอง"). Grounded in craft, effort, and circumstance, never doom or get-rich fantasy. Warm, not cruel — a hard truth delivered kindly.
ANTI-FINGERPRINT (the corpus is generated in bulk, so vary it): do NOT lean on one closing formula — rotate how you land the empowerment (sometimes "yours to make", sometimes end on the image, on a question, on the next concrete act) so no single closer dominates the set. Vary the narrator tag — not every line should open "The cards show…/ไพ่เห็นว่า…"; let the card speak directly and drop the tag sometimes. Use em-dashes sparingly (at most one per answer; prefer full stops). Vary sentence openings and rhythm from card to card.
