#!/usr/bin/env python3
"""Post-process one build-career-question workflow output:
  - read the workflow result file ({"result": {...dataset...}}), strip _meta
  - prune non-canonical keys, validate completeness (78 cards x up/rev x positions x en/th)
  - write assets/data/career-readings/<slug>-reading.json
  - register key->path in /career/reading/index.html QUESTION_DATASETS (only if that app exists)

Usage: postprocess-career-question.py <result_file> <slug> <key>
"""
import json, os, re, sys

REPO = "/Users/kitikornrakhangthong/projects/veilatarot.com"
APP = os.path.join(REPO, "career/reading/index.html")
...
def main():
    out_file, slug, key = sys.argv[1], sys.argv[2], sys.argv[3]

    w = json.load(open(out_file))
    d = w["result"] if isinstance(w.get("result"), dict) else (
        w if "cards" in w else json.loads(w["result"]))
    d.pop("_meta", None)
    ...
    rel = f"assets/data/career-readings/{slug}-reading.json"
    path = os.path.join(REPO, rel)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    json.dump(d, open(path, "w"), ensure_ascii=False, indent=2)
    size = os.path.getsize(path)

    wired = "app-not-built-yet"
    if os.path.exists(APP):
        html = open(APP).read()
        entry = f"  '{key}': '/assets/data/career-readings/{slug}-reading.json'"
        if f"'{key}':" not in html:
            m = re.search(r"(const QUESTION_DATASETS = \{.*?)(\n\};)", html, re.S)
            if not m:
                raise SystemExit("could not locate QUESTION_DATASETS block in app")
    ...
    print(f"OK {slug}: {size} bytes, 78 cards, {wired}")
